  ...
  useEffect(() => {
    const fetchMenProducts = async () => {
      try {
        ...
        const { data } = await axios.get("/api/product/men", {
        ...
        if (data.success) {
          ...
          setError(data.message || "Failed to fetch men's products");
        }
      } catch (err) {
        setError("Error fetching men's products");
        console.error(err);
      } finally {
        setLoading(false);
      }
    ...
    return (
      <div className="min-h-screen bg-white flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="mt-4 text-old-olive">Loading men's collection...</p>
        </div>
      </div>
    );
  }
        ...
        <div className="flex flex-col-reverse md:flex-row items-center justify-between bg-[#E6E9F2] py-8 md:px-14 px-5 mt-6 rounded-xl">
          <div className="md:pl-8 mt-10 md:mt-0">
            ...
            <p className="max-w-lg text-gray-600 mt-4">
              Discover our curated selection of men's fashion. From casual essentials to sophisticated formal wear, find everything that defines your style.
            </p>
        ...
        <div className="max-w-7xl mx-auto px-4 md:px-10 lg:px-16 py-12">
          ...
          {products && products.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {(() => {
                ...
                return Object.entries(categories).map(([categoryName, categoryProducts]) => (
                  <Link 
                    key={categoryName}
                    href={`/men?category=${encodeURIComponent(categoryName)}`}
            ...
            <div className="text-center py-12">
              <p className="text-gray-600 text-lg">No products available in men's collection.</p>
              <Link 
                href="/"
